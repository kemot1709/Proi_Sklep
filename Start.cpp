...
void jadymy_z_szalotem(int n, int m, int o) {   // kasy, pracownicy, towary
    std::fstream plik;
    plik.open("przzebieg.txt", std::ios::out);
    const int podstawa = time(nullptr);
    std::mt19937_64 e1(podstawa);
//    std::random_device e1;
    ...
    for(int i = 1;; i++) { // licznik dni
        ...
        for(int j = 1; j <= 120; j++) { // jeden tick - 6 min
            std::cout << std::setfill('0') << std::setw(2) << std::endl << 8 + (j / 10) << ":";
            std::cout << std::setfill('0') << std::setw(2) << (j % 10) * 6 << "\n\n";
            plik << std::setfill('0') << std::setw(2) << std::endl << 8 + (j / 10) << ":";
            plik << std::setfill('0') << std::setw(2) << (j % 10) * 6 << "\n\n";
            int czas_komend = 0;
            // wejscie klientow
            if(j < 111) {
                ...
                if(r > 0) {
                    ...
                    plik << "Do sklepu wchodzi " << r << " klientow" << std::endl;
                    Sleep(100);
                }
            }
                ...
                if(prac.pracownik_f_stan(0) != 0) { // sprawdzenie czy pracownik ma wolne
                    if(prac.pracownik_f_stan(3) != 0) { // sprawdzenie czy pracownik jest zajety i skorygowanie czasu pracy
                        ...
                        if(losuj_zajecie < 25 && prac.pracownik_f_stan(1) < 4) { // pracownik idzie na kase
                            ...
                            plik << "Pracownik " << k + 1 << " idzie na kase nr " << wolna_kasa + 1 << std::endl;
                        }
                            ...
                            plik << "Pracownik " << k + 1 << " idzie wykladac towar" << std::endl;
                        }
                            ...
                            plik << "Pracownik " << k + 1 << " idzie na przerwe" << std::endl;
                        }
            ...
            for(int k = 0; k < p; k++) {
                ...
                if(klie.klient_f_zajecie() != en_k_wyszedl && klie.klient_f_zajecie() != en_k_przy_kasie) { // sprawdza czy klient jeszcz jest w sklepie lub juz przy kasie
                    ...
                    if(losuj_zajecie < 15 && klie.klient_ilosc_zakupow() > 0) { // idzie do kasy
                        ...
                        plik << "Klient " << k + 1 << " podchodzi do kolejki do kasy nr " << kol_kasa + 1 << std::endl;
                    }
                        ...
                        for(int l = 0; l < m; l++) { // szukanie wolnego pracownika
                            ...
                            if(prac.pracownik_f_zajecie() == en_p_chodzi || prac.pracownik_f_zajecie() == en_p_sklep) {
                                ...
                                plik << "Klient " << k + 1 << " rozmawia z pracownikiem nr " << l + 1 << std::endl;
                                break;
                            }
                        }
                    }
                        ...
                        plik << "Klient " << k + 1 << " sprawdza cene towaru nr " << wybrany_tow << ", wynosi ona " << supermarket.sklep_towar(wybrany_tow).towar_f_cena_brutto() << std::endl;
                    }
                        ...
                        plik << "Klient " << k + 1 << " bierze do koszyka produkt nr " << wybrany_tow + 1 << std::endl;
                    }
                    czas_komend++;
                }
            ...
            for(int k = 0; k < n; k++) {
                ...
                if(kasa.kasa_f_stan() == true) { // sprawdzenie czy kasa jest otwarta
                    if(kasa.kasa_ilosc_osob() > 0) { // sprawdza czy ktos jest w kolejce
                        int pom = kasa.kasa_pierwszy();
                        double x = kasa.kasa_usun_osobe(supermarket.sklep_klient(kasa.kasa_pierwszy()));
                        if(x >= 0) {
                            std::cout << "Klient " << pom + 1 << " wyszedl i zaplacil " << x << std::endl;
                            plik << "Klient " << pom + 1 << " wyszedl i zaplacil " << x << std::endl;
                        }
                        czas_komend++;
                    }
                }
            ...
            Sleep(200 * czas_komend + 1000); // czas oczekiwania - przerwa na czytanie zdarzen
        }
        std::cout << "Zarobek sklepu w dniu " << i << " : " << supermarket.sklep_f_utarg() << std::endl;
        plik << "Zarobek sklepu w dniu " << i << " : " << supermarket.sklep_f_utarg() << std::endl;
        Sleep(1000);
    }
}
